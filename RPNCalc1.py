...
class RPNCalc(object):
	...
	def readMe(self):
		main = self.root
		readmeFRAME = Frame(main,width=600,height=450,bg='#bbbbbb')
		readmeFRAME.place(x=0,y=0)
		self.visualFrames.append(readmeFRAME)
		readThis1 = Label(main,text='Simple GUI by Chris(Moon 🌙#5413)\n'
					+'Ignore Calc Styling, we are using the same grid\n'
					+'Sudoku will run in another window for better, quicker grid.\n'
					, fg='#bbbbbb', bg='#2c2f33', font= 'arial 12 bold')
		readThis1.grid(row = 0, column = 0, pady = 10, padx = 10)
		self.visualFrames.append(readThis1)
	#Help page..
	def helpMe(self):
		main = self.root
	...
	def createStack(self):
		main = self.root
		rpnFRAME = Frame(main,width=600,height=450,bg='#bbbbbb')
	...
	def createCalc(self):
		...
		more_ = Button(main,text="More labels here soon",width=20)
